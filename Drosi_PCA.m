function Drosi_PCA(Red_mutant_groups_13tun,mut)
...
%		MUT: ordinal number of the selected genotpye in the
%			 RED_MUTANT_GROUPS_13TUN struct
%
...
% Extract data of the selected genotpye
speedStPerdrosi2sec = Red_mutant_groups_13tun(mut).age_groups.speed_st_perdrosi_2sec;
...
y = speedStPerdrosi2sec(:,180:300); % cut the time window of the reaction
...
for pcaind = 1:clustnum
    ...
    imagesc(speedStPerdrosi2sec(Clusters == pcaind,:));
    ylabel('Trials');
    xlabel('Time');
    xlim([100 350]);
    setmyplot_balazs
    figure;  % figure for mean speed plot
    plot(mean(speedStPerdrosi2sec(Clusters == pcaind,:)),'b-');
